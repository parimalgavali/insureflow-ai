...
7. List claims with `GET /claims`.
8. Read the claim with `GET /claims/{claimNumber}`.
9. Read timeline events with `GET /claims/{claimNumber}/events`.
10. Add a note with `POST /claims/{claimNumber}/notes`.
11. Add document metadata with `POST /claims/{claimNumber}/documents`.
12. Change claim status with `POST /claims/{claimNumber}/status`.
...
- Claim list reads return newest reported claims first for adjuster queue display.
